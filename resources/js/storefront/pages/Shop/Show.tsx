...
function RelatedCard({ product }: { product: ProductCard }) {
  ...
  return (
    <Link href={`/shop/${product.slug}`} className="group block" style={{ textDecoration: 'none' }}>
      <div className="kb-card overflow-hidden hover:shadow-md transition-shadow" style={{ borderRadius: 10 }}>
...
export default function ShopShow({ product, related }: Props) {
  ...
  return (
    <Layout>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-5 pb-28 lg:pb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10">
          ...
          <div>
            {/* Main image */}
            <div className="rounded-xl overflow-hidden bg-gray-100 relative" style={{ aspectRatio: '1/1' }}>
              ...
              <button
                ...
                style={{ background: wishlisted ? '#fff0f0' : 'rgba(255,255,255,.9)', color: wishlisted ? '#ef4444' : '#9ca3af' }}
                title={wishlisted ? 'Remove from wishlist' : 'Add to wishlist'}>
                <svg className="w-4.5 h-4.5" fill={wishlisted ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex gap-2 mt-2.5 overflow-x-auto pb-1">
                {product.images.map((img, i) => (
                  <button key={img.id} onClick={() => setActiveImage(i)}
                    className={`shrink-0 w-14 h-14 rounded-lg overflow-hidden border-2 transition-colors ${i === activeImage ? 'border-blue-500' : 'border-transparent hover:border-gray-300'}`}>
          ...
          <div className="space-y-4">
            {product.brand && (
              <Link href={`/shop/b/${product.brand.slug}`} className="text-xs font-semibold uppercase tracking-wide hover:underline" style={{ color: 'var(--kb-primary)' }}>
                {product.brand.name}
              </Link>
            )}

            <h1 className="text-xl md:text-2xl font-bold leading-tight" style={{ color: 'var(--kb-ink)' }}>{product.name}</h1>

            {/* Rating row */}
            {product.reviews_count > 0 && product.reviews_avg && (
              <div className="flex items-center gap-2">
                <span className="text-yellow-400 text-sm">{'★'.repeat(Math.round(product.reviews_avg))}{'☆'.repeat(5 - Math.round(product.reviews_avg))}</span>
                <span className="text-xs text-gray-500">{Number(product.reviews_avg).toFixed(1)} ({product.reviews_count} reviews)</span>
              </div>
            )}

            {/* Price */}
            <div className="flex items-baseline gap-3 py-2 border-y" style={{ borderColor: 'var(--kb-border)' }}>
              <span className="text-2xl md:text-3xl font-extrabold" style={{ color: 'var(--kb-primary)' }}>৳{displayPrice.toLocaleString()}</span>
              {comparePrice && <span className="text-base text-gray-400 line-through">৳{comparePrice.toLocaleString()}</span>}
              {discount !== null && discount > 0 && (
                <span className="text-sm font-bold px-2 py-0.5 rounded-full" style={{ background: '#fef2f2', color: '#ef4444' }}>-{discount}% OFF</span>
              )}
            </div>
            ...
            {product.has_variants && groupedOptions.map(attr => (
              <div key={attr}>
                <p className="text-sm font-semibold text-gray-700 mb-2">
                  {attr}: <span className="font-normal text-gray-500">{selectedOptions[attr]}</span>
                </p>
                    ...
                    return (
                      <button key={val}
                        ...
                        className={`px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors ${
                          unavailable ? 'border-gray-100 text-gray-300 bg-gray-50 cursor-not-allowed'
                          : outOfStock ? 'border-gray-200 text-gray-300 bg-gray-50 cursor-not-allowed'
            ...
            <div className="flex items-center gap-4 text-xs text-gray-400">
              ...
              {isInStock
                ? isLowStock
                  ? <span className="flex items-center gap-1 text-orange-500 font-semibold"><span className="w-2 h-2 rounded-full bg-orange-400" />Only {stockQty} left!</span>
                  : <span className="flex items-center gap-1 text-green-600 font-semibold"><span className="w-2 h-2 rounded-full bg-green-500" />In Stock</span>
                : <span className="flex items-center gap-1 text-red-500 font-semibold"><span className="w-2 h-2 rounded-full bg-red-400" />Out of Stock</span>
              }
            </div>
            ...
            <div className="hidden lg:block space-y-2">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                  <button onClick={() => setQty(q => Math.max(1, q - 1))} className="w-10 h-11 flex items-center justify-center text-gray-600 hover:bg-gray-50 text-lg">−</button>
                  <span className="w-10 text-center text-sm font-semibold">{qty}</span>
                  <button onClick={() => setQty(q => q + 1)} className="w-10 h-11 flex items-center justify-center text-gray-600 hover:bg-gray-50 text-lg">+</button>
                </div>
                {/* Wishlist */}
                <button
                  onClick={() => toggleWishlist({ id: product.id, slug: product.slug, name: product.name, price: product.price, compare_at_price: product.compare_at_price, image_url: product.images[0]?.url ?? null, category: null })}
              ...
              <div className="divide-y divide-gray-100 border border-gray-100 rounded-xl overflow-hidden text-sm">
                {product.shipping_included && (
                  <div className="flex items-center gap-3 px-4 py-3">
                    <svg className="w-4 h-4 text-green-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8l1 13h12l1-13M10 12h4"/></svg>
                    <span className="font-medium text-green-700">Free Shipping</span>
                    <span className="text-gray-500 text-xs">Delivery included</span>
                  </div>
                )}
                {product.warranty && (
                  <div className="flex items-start gap-3 px-4 py-3">
                    <svg className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                    <div><span className="font-medium text-gray-800">Warranty</span><span className="text-gray-500 ml-2 text-xs">{product.warranty}</span></div>
                  </div>
                )}
                {(product.return_policy || site.globalReturnPolicy) && (
                  <div className="flex items-start gap-3 px-4 py-3">
                    <svg className="w-4 h-4 text-green-600 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                    <div>
                      <span className="font-medium text-gray-800">Returns</span>
                      <p className="text-gray-500 text-xs mt-0.5 whitespace-pre-line">{product.return_policy || site.globalReturnPolicy}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-2">
                {site.productWhatsappEnabled && whatsappUrl && (
                  <a href={whatsappUrl} target="_blank" rel="noopener noreferrer"
                    className="flex-1 kb-btn kb-btn-lg flex items-center justify-center gap-2 font-medium text-sm"
                ...
                <tbody>
                  {product.specifications.map((spec, i) => (
                    <tr key={i} className={i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                      <td className="px-4 py-2.5 font-medium text-gray-700 w-36 sm:w-44 border-r border-gray-200 text-xs sm:text-sm">{spec.key}</td>
                      <td className="px-4 py-2.5 text-gray-600 text-xs sm:text-sm">{spec.value}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="mt-8">
          <h2 className="text-lg font-bold text-gray-900 mb-4">
            Customer Reviews
            {product.reviews_count > 0 && <span className="text-sm font-normal text-gray-500 ml-2">({product.reviews_count})</span>}
          </h2>
          {product.reviews_count > 0 && product.reviews_avg && (
            <div className="flex items-center gap-3 mb-5">
              <span className="text-4xl font-bold text-gray-900">{Number(product.reviews_avg).toFixed(1)}</span>
              <div>
                <div className="text-yellow-400 text-lg">{'★'.repeat(Math.round(product.reviews_avg))}{'☆'.repeat(5 - Math.round(product.reviews_avg))}</div>
                <p className="text-xs text-gray-500">{product.reviews_count} review{product.reviews_count !== 1 ? 's' : ''}</p>
              </div>
            </div>
          )}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow-lg">
        ...
        <div className="flex items-center gap-2 px-4 pt-2.5 pb-1">
          ...
          <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
            <button onClick={() => setQty(q => Math.max(1, q - 1))} className="w-8 h-8 flex items-center justify-center text-gray-600 hover:bg-gray-50 text-base font-medium">−</button>
            <span className="w-8 text-center text-sm font-semibold">{qty}</span>
            <button onClick={() => setQty(q => q + 1)} className="w-8 h-8 flex items-center justify-center text-gray-600 hover:bg-gray-50 text-base font-medium">+</button>
          </div>
          {/* Stock indicator */}
          {isInStock
            ? isLowStock
              ? <span className="text-xs text-orange-500 font-medium">Only {stockQty} left!</span>
              : <span className="text-xs text-green-600 font-medium flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-500" />In Stock</span>
          ...
          <button
            ...
            style={{ color: wishlisted ? '#ef4444' : '#9ca3af', background: wishlisted ? '#fff0f0' : '#fff' }}>
            <svg className="w-4.5 h-4.5" fill={wishlisted ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
            </svg>
          </button>
        ...
        <div className="flex gap-2 px-4 pb-4">
          <button onClick={handleAddToCart} disabled={!isInStock}
            className={`flex-1 kb-btn flex items-center justify-center gap-2 font-semibold text-sm py-3 rounded-xl disabled:opacity-50 ${added ? 'bg-green-600 text-white' : 'kb-btn-primary'}`}>
            {added
          ...
          <button onClick={handleBuyNow} disabled={!isInStock}
            className="flex-1 kb-btn flex items-center justify-center gap-2 font-semibold text-sm py-3 rounded-xl disabled:opacity-50"
            style={{ background: '#0f172a', color: '#fff' }}>
          ...
          <Link href="/checkout"
            className="w-12 flex items-center justify-center rounded-xl border border-gray-200 bg-white"
