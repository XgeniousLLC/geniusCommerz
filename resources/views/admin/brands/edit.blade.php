...
<div class="row" style="gap:14px;margin-bottom:22px;flex-wrap:wrap">
    ...
    <div class="grow" style="min-width:180px">
        <div class="breadcrumb"><a href="{{ route('admin.brands.index') }}">Brands</a> / Edit</div>
        <h2 class="display" style="font-size:24px;letter-spacing:-0.03em">{{ $brand->name }}</h2>
    </div>
    ...
    <div class="card pad">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px" class="grid-2">
            <div class="field">
                ...
                <input class="input mono" type="text" name="slug" value="{{ old('slug', $brand->slug) }}" readonly
                       style="background:var(--surface-2);cursor:not-allowed;color:var(--text-muted)">
                <span class="faint" style="font-size:12px">Locked after creation.</span>
            </div>
            <div class="field">
                <span class="lbl">Website</span>
                <div class="input-prefix">
                    <span style="font-size:11px">https://</span>
                    <input class="input" type="text" name="website" value="{{ old('website', $brand->website) }}" placeholder="brand.com" style="padding-left:52px">
                </div>
        ...
        <div class="field">
            ...
            <textarea class="textarea" name="description" rows="3">{{ old('description', $brand->description) }}</textarea>
        </div>
    </div>
</div>
...
<div class="col-gap">
    ...
    <div class="card pad">
        ...
        <label class="between" style="cursor:pointer" x-data="{ on: {{ old('is_active', $brand->is_active) ? 'true' : 'false' }} }">
            <div>
            ...
            <input type="checkbox" name="is_active" value="1" style="display:none" :checked="on" {{ old('is_active', $brand->is_active) ? 'checked' : '' }}>
