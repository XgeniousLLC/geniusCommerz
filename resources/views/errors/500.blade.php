...
<head>
    ...
    <title>500 — Something went wrong · {{ \App\Models\SiteSetting::get('general.site_name', 'geniusCommerz') }}</title>
...
<body>
    ...
    <div style="position:relative;overflow:hidden;padding:clamp(48px,8vw,80px) 0;text-align:center;z-index:1">
        <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;pointer-events:none;overflow:hidden">
            <span style="font-family:var(--av-display);font-size:clamp(160px,32vw,420px);font-weight:600;color:rgba(31,26,21,.035);letter-spacing:-.04em;user-select:none;white-space:nowrap;font-style:italic;line-height:1">500</span>
        </div>
        <div class="wrap" style="position:relative">
            <div class="eyebrow" style="margin-bottom:14px">Server error</div>
            <h1 style="font-family:var(--av-display);font-size:clamp(30px,4.5vw,48px);font-weight:400;letter-spacing:-.012em;line-height:1.04;margin:0 0 14px;color:var(--av-ink)">Something went wrong.</h1>
            <p style="color:var(--av-muted);font-size:14.5px;line-height:1.7;margin:0 auto 28px;max-width:520px">
                Our server ran into an unexpected error. We're already notified and working on a fix. Please try again in a few minutes.
            </p>
            <div style="display:flex;gap:12;justify-content:center;flex-wrap:wrap">
                <a href="/" class="btn-primary">Back to home</a>
                <a href="/shop" class="btn-ghost">Browse products</a>
            </div>
            @if(!empty($exception) && $exception->getMessage() && app()->environment('local'))
                <div style="margin:24px auto 0;max-width:560px;text-align:left;padding:14px 16px;background:var(--av-paper);border:1px solid var(--av-line-soft);font-size:12px;color:var(--av-muted);font-family:ui-monospace,monospace;word-break:break-word">{{ $exception->getMessage() }}</div>
            @endif
        </div>
    </div>
